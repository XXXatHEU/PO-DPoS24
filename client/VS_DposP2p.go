...
func VS_DposP2phandleConnection(conn net.Conn) {
	defer conn.Close()
	...
	go func() {
		...
		if errdposGetAskJson != nil {
			...
			default:
				Log.Debug("VS_DposP2phandleConnection中的没有处理的语句")
				// 如果没有匹配的值，则执行默认语句块
			}
		}
	}()
...
//验证交易
func VS_DposValidthetx(nodeChan chan dposJsonStruct, hisTx dposJsonStruct) {
	//定义发送的的请求为初始化
	var validationResult dposJsonStruct
	validationResult.StringData = "节点发送了一笔恶意交易"
	validationResult.Comm = VS_ValidResult
	...
	if hisTx.MyNodeType == healthyNode {
		...
		VS_NotifyMaliciousNode(InitNum) //举报
	}
	nodeChan <- validationResult
}

//举报恶意节点
func VS_NotifyMaliciousNode(InitNum int) {
	var MalicNode dposJsonStruct
	MalicNode.Comm = VS_NotifyMaliciousNodeComm
