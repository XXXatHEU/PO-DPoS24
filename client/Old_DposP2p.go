...
func Old_DposP2phandleConnection(conn net.Conn) {
	defer conn.Close()
	...
	go func() {
		...
		if errdposGetAskJson != nil {
			...
			default:
				Log.Debug("Old_DposP2phandleConnection中的没有处理的语句")
				// 如果没有匹配的值，则执行默认语句块
			}
		}
	}()
...
//验证交易
func Old_DposValidthetx(nodeChan chan dposJsonStruct, hisTx dposJsonStruct) {
	//定义发送的的请求为初始化
	var validationResult dposJsonStruct
	validationResult.StringData = "节点发送了一笔恶意交易"
	validationResult.Comm = Old_ValidResult
	...
	if hisTx.MyNodeType == healthyNode {
		...
		Old_NotifyMaliciousNode(InitNum) //举报
	}
	nodeChan <- validationResult
}

//举报恶意节点
func Old_NotifyMaliciousNode(InitNum int) {
	var MalicNode dposJsonStruct
	MalicNode.Comm = Old_NotifyMaliciousNodeComm
