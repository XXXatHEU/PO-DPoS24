...
//共有结构体上述
var Old_AskForChan = make(chan dposJsonStruct)

var Old_myGroupNum = 0
var Old_GrupMclic = make(map[int][]int) //每组恶意节点的集合
var Old_inMclieGrup = make(map[int]int) //自己组内恶意节点的集合 key是节点数  value是在组内的下标
var Old_inGrup = make(map[int]bool)     //自己组内所有节点的集合
...
//询问我该向哪个集群进行验证
func Old_AskForValidation() string {
...
func Old_ReplyForvaliNodeControl(replydata dposJsonStruct) {
	Old_AskForChan <- replydata
}

//找谁进行验证 （发送给谁）
func Old_AskForSomeOneValid(conn net.Conn) {
	...
	for {
		...
		//1.找谁进行验证 要求返回ip
		msg := Old_AskForValidation()

		connWrite(conn, []byte(msg))

		//阻塞等待返回  需要返回ip
		dataStruct = <-Old_AskForChan
		nodeComm := dataStruct.Old_ReplyForvaliNode
		...
		if nodeComm == int(SelfValid) {
			...
			//os.Exit(0) 没有拉黑选项
		} else if nodeComm == int(Old_ReplyAskForVali) {
	...
	ValidMessage.Comm = Old_ValidmyData
