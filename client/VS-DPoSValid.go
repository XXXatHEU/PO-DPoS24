...
//共有结构体上述
var VS_AskForChan = make(chan dposJsonStruct)

var VS_myGroupNum = 0
var VS_GrupMclic = make(map[int][]int) //每组恶意节点的集合
var VS_inMclieGrup = make(map[int]int) //自己组内恶意节点的集合 key是节点数  value是在组内的下标
var VS_inGrup = make(map[int]bool)     //自己组内所有节点的集合
...
//询问我该向哪个集群进行验证
func VS_AskForValidation() string {
...
func VS_ReplyForvaliNodeControl(replydata dposJsonStruct) {
	VS_AskForChan <- replydata
}

//找谁进行验证 （发送给谁）
func VS_AskForSomeOneValid(conn net.Conn) {
	...
	for {
		...
		//1.找谁进行验证 要求返回ip
		msg := VS_AskForValidation()

		connWrite(conn, []byte(msg))

		//阻塞等待返回  需要返回ip
		dataStruct = <-VS_AskForChan
		nodeComm := dataStruct.VS_ReplyForvaliNode
		...
		if nodeComm == int(SelfValid) {
			...
			//os.Exit(0) 没有拉黑选项
		} else if nodeComm == int(VS_ReplyAskForVali) {
	...
	ValidMessage.Comm = VS_ValidmyData
